...
namespace Graphics3DSample
{
    ...
    public class Spaceship
    {
        ...
        public void Draw()
        {
            for (int i = 0; i < 2; i++)
            {
                int dist = i == 0 ? 750 : -750;
                Matrix translate = Matrix.CreateTranslation(new Vector3(dist, 0, 0));
                Matrix transform = Matrix.Multiply(translate, Rotation);
                // Set the world matrix as the root transform of the model.
                spaceshipModel.Root.Transform = transform;
